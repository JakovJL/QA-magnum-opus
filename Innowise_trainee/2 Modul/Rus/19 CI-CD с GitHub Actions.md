...
Тестирование — одна из главных причин, почему вообще существует CI/CD.

**Где тесты обычно запускаются в pipeline:**
- **unit tests** — рано, как быстрая проверка качества
- **API и integration tests** — после build или deploy в test environment
- **UI / E2E tests** — позже, потому что они медленнее и более хрупкие

**Как тесты влияют на pipeline:**
- успешные тесты позволяют pipeline идти дальше
- упавшие тесты могут остановить build, заблокировать deploy или merge

**Тестирование как quality gate:**
- для pull request часто нужны зелёные тесты перед merge
- smoke tests могут запускаться сразу после deployment
- если критичные тесты падают после deployment, команда может сделать rollback автоматически или вручную

**Цель:** находить дефекты как можно раньше и не давать сломанному коду идти дальше.
